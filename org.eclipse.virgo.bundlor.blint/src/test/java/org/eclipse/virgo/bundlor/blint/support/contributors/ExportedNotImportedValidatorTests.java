...
public class ExportedNotImportedValidatorTests {

    private final ExportedNotImportedValidator validator = new ExportedNotImportedValidator();

    @Test
    public void valid() {
        ManifestContents manifest = new SimpleManifestContents();
        manifest.getMainAttributes().put("Export-Package", "test.package");
        manifest.getMainAttributes().put("Import-Package", "test.package2");
